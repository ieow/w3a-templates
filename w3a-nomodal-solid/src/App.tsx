...
const App: Component = () => {
  ...
  const [web3auth, setWeb3auth] = createSignal<
    web3nomodal.Web3AuthNoModal | undefined
  >();
  ...
  onMount(async () => {
    ...
    try {
      const chainConfig = {
        chainNamespace: web3base.CHAIN_NAMESPACES.SOLANA,
      ...
      const web3auth = new web3nomodal.Web3AuthNoModal({
        clientId,
        privateKeyProvider,
        web3AuthNetwork: web3base.WEB3AUTH_NETWORK.SAPPHIRE_MAINNET,
      });
      ...
      const openloginAdapter = new web3openlogin.OpenloginAdapter({
        privateKeyProvider,
        adapterSettings: {
          uxMode: web3base.UX_MODE.REDIRECT,
        },
      });
  ...
  const login = async () => {
    ...
    const web3authProvider = await auth.connectTo(
      web3base.WALLET_ADAPTERS.OPENLOGIN,
      {
