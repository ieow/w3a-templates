...
import * as web3solanaprovider from "@web3auth/solana-provider";
// import { Metaplex, keypairIdentity } from "@metaplex-foundation/js";

export default class SolanaRpc {
  private provider: web3base.IProvider;

  constructor(provider: web3base.IProvider) {
    this.provider = provider;
  }

  getAccounts = async (): Promise<string[]> => {
    try {
      const solanaWallet = new web3solanaprovider.SolanaWallet(this.provider);
  ...
  signVersionedTransaction = async (): Promise<VersionedTransaction> => {
    try {
      ...
      const signedTx =
        await solanaWallet.signTransaction<VersionedTransaction>(transaction);
      return signedTx;
    } catch (error) {
      throw error;
    }
